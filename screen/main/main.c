...
#define CONFIG_WIDTH 135 //135
#define CONFIG_HEIGHT 240  //240
...
void AnimationTest(TFT_t *dev, int width, int height)
{
	int x = 10;
	int length = 5;
	for (int y = 0; y < height; y += 20)
	{
		...
		lcdDrawFinish(dev);
		vTaskDelay(1);
		lcdDrawFillRect(dev, x, y, x + length, y + length, BLUE);
		vTaskDelay(1);
		lcdDrawFinish(dev);
		vTaskDelay(2);
	}
...
#define SGM2578_ENABLE_GPIO 27
	...
	while (1)
	{
		AnimationTest(&dev, CONFIG_WIDTH, CONFIG_HEIGHT);
